...
class QEDVFSScheduler:
    ...
    def get_feature_set(self, job_name, kmeans_corelated):
        ...
        ## If we need to run with the kmeans corelated profile data
        if kmeans_corelated:
            cluster_info = pd.read_csv(self.data_directory + "kmeans/apps_selection_basedon_clusters.csv")
            corelated_app = cluster_info[cluster_info['Application_name'] == job_name].corelated_app.values[0]
            print(corelated_app)
            print("Corealted App for {} is {}".format(job_name, corelated_app))
            df = pd.read_csv(
                os.path.join(self.data_directory + "combined_metrics/" + corelated_app + "_dmon_nvprof_combined.csv"))
        else:
    ...
    def predict_power_time(self, name, pwr_prediction_model, time_prediction_model):
        ...
        try:
            kmeans_corelated = True
            feature_set = self.get_feature_set(name, kmeans_corelated)
    ...
    def schedule_workload(self):
        ...
        for job in job_list:
            ...
            if job.arrival_time >= elapsed_time:
                ...
                print("ISCLOCKSET: {} Job: {} Selected Application Clock: {} - {}".format(is_clock_set, job.name,
                                                                                          job.mem_clock, job.sm_clock))
                print(
